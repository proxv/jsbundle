...
function _findBundles(dirs, env, callback) {
  if (!Array.isArray(dirs)) {
    dirs = [ dirs ];
  }
  var bundleNameToConfigMap = {};
  async.parallel(dirs.map(function(cwd) {
    return function(cb) {
      exec("find . -follow -type f -iname package.json '!' -iwholename '*/node_modules/*'", { cwd: cwd }, function (err, stdout, stderr) {
        ...
        _(packageJsonFiles).each(function (file) {
          ...
          try {
            ...
            if (name) {
              name = name + '.js';
              try {
                console.error('Found bundle: ' + name + ' at: ' + dir);
