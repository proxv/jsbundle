...
Module.prototype = {
  ...
  _errorMessageWithContext: function(message, src, lineNumber) {
    var context = ':\n\n        ' + src.split(/\n|\r\n/)[lineNumber - 1];
    return message + context + '\n\n    at ' + this._filename + (lineNumber ? ':' + lineNumber : '');
  },
  ...
  _compileExtraRequires: function(extraRequires) {
    ...
    _(extraRequires).each(function (value, key) {
      try {
        ...
        throw new Error(itself._errorMessageWithContext(e.message + ', added via extraRequires'));
      }
          ...
          if (paramToken.start.type === 'string') {
            try {
              ...
              if (resolvedName.charAt(0) !== '/') {
                throw new Error(itself._errorMessageWithContext("can't handle native module require", src, this[0].start.line + 1));
              }
