...
Module.prototype = {
  ...
  _excludeFromExtraRequires: function() {
    return _.reduce(this._options.excludeDirsFromExtraRequires || [], function(memo, excludeDir) {
      return memo || (this._filename.indexOf(path.resolve(path.dirname(this._options.entryFile), excludeDir)) >= 0);
    }, false, this);
  },

  _compileExtraRequires: function(extraRequires) {
    if (this._excludeFromExtraRequires()) {
      return '';
    }
        ...
        if (funcName === 'require') {
          ...
          if (paramToken.start.type === 'string') {
            ...
            try {
              ...
              var errorLine = this[0] && this[0].start && this[0].start.line + 1;
              itself.error = new Error(itself._errorMessageWithContext(e.message, src, errorLine));
              throw itself.error;
            }
