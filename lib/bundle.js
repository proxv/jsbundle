...
/*
   parameters:
      ...
      entryFile:          string path to the file with which to start bundling

      configOrConfigFile: if a string, the path to a JSON config file
                          if an object, the config options as key-value pairs

      configDir:          if specified, the directory from which to resolve relative paths in the configuration
                          defaults to either the directory containing the config file, if known,
                          or the directory of the entryFile
*/
function bundle(entryFile, configOrConfigFile, configDir) {
  entryFile = path.resolve(entryFile);

  var options = {};
  if (typeof configOrConfigFile === 'string') {
    options = JSON.parse(fs.readFileSync(configOrConfigFile, 'utf-8'));
    configDir = configDir || path.dirname(configOrConfigFile);
  } else if (typeof configOrConfigFile === 'object') {
    options = configOrConfigFile;
    configDir = configDir || path.dirname(entryFile);
  }
  ...
  var src = template.compile('bundle', {
    beforeBundle: _compileStatementArray(options.beforeBundle),
    afterBundle: _compileStatementArray(options.afterBundle),
