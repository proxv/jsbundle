...
function _filterRequires(options) {
  var requires = [];
  _(options.filters).each(function(filter) {
...
function bundle(options) {
  var entryFile = path.resolve(options.entryFile);

  var counter = 0;
  var files = [ entryFile ].concat(_filterRequires(options));
  ...
  _(modules).each(function(module, id) {
    id = (options.mangleNames ? Number(id) : id);
