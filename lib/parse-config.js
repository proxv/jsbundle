...
function parseConfig(configFile, env) {
  ...
  config.devCdnPort = Number(config.devCdnPort);

  var devCdnPaths = config.devCdnPaths || [ '.' ];
  _(devCdnPaths).each(function(devCdnPath, i) {
    devCdnPaths[i] = path.resolve(configDir, devCdnPath);
  });
  config.devCdnPaths = devCdnPaths;
